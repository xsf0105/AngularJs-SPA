...
var igrowhelper = require('./scripts/igrowhelper').igrowHelper;
...
gulp.task('sass', function() {
	...
	igrowhelper.log('sass compile success');
});
...
gulp.task('clean', ['backup'], function() {
	igrowhelper.log('Starting clean...');
	return del(['./release/**']).then(function(paths) {
		igrowhelper.log('All files cleaned!');
	});
})
...
gulp.task('release', ['override'], function() {
	gulp.src('./release/**')
		.pipe(zip('cheerhi_web_latest.zip'))
